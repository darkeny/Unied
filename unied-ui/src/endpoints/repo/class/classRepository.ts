...
import type { ClassModel } from "../../model/class/class";
export default class ClassRepository implements IRepository<ClassModel> {
  uri = "/classes";

  async readById(id: number): Promise<IApiResponse<ClassModel[]>> {
    try {
      const request = await axios.get(URLBASE + this.uri + "/" + id.toString());
      const response: IApiResponse<ClassModel[]> = {
  ...
  async readAll(): Promise<IApiResponse<ClassModel[]>> {
    try {
  ...
  async create(student: ClassModel): Promise<IApiResponse<ClassModel[]>> {
    try {
  ...
  async update(student: ClassModel): Promise<IApiResponse<ClassModel[]>> {
    try {
  ...
  async delete(id: number): Promise<IApiResponse<ClassModel[]>> {
    try {
